...
Source: https://platform.openai.com/docs/api-reference/usage/completions
...
- `batch` (query, boolean, optional): If `true`, return batch jobs only. If `false`, return non-batch jobs only. By default, return both.
- `group_by` (query, array<string>, optional): Group the usage data by the specified fields. Support fields include `project_id`, `user_id`, `api_key_id`, `model`, `batch`, `service_tier` or any combination of them.
...
## Returns
A list of paginated, time bucketed [Completions usage](completions_object.md) objects.
