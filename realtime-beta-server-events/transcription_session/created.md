...
Source: https://platform.openai.com/docs/api-reference/realtime-beta-server-events/transcription_session/created

Returned when a transcription session is created.

## Properties
- `event_id` (string, required): The unique ID of the server event.
- `type` (string, required): The event type, must be `transcription_session.created`. Enum: 'transcription_session.created'.
- `session` (object, required): A new Realtime transcription session configuration.
