...
- `thread` (object, optional): Options to create a new thread. If no thread is provided when running a
...
- `instructions` (string, optional): Override the default system message of the assistant. This is useful for modifying the behavior on a per-run basis.
